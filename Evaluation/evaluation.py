...
def run_inference(features, encoder,
    ...
    if full_sequence:
        ...
        sliding_windows = AAF_feature_extraction.extract_sliding_windows(features)
...
class TripletScoreFetcher:
    ...
    def __call__(self, i):
        ...
        anchor, pos, neg = AAF_feature_extraction.get_triplet_features(
            self.spk_to_utts)
...
def compute_eer(labels, scores):
    ...
    while threshold < 1.0:
        ...
        accrate = ((len(labels) - sum(fa)) + (len(labels) - sum(fr))) / (len(labels) * 2)
        far = sum(fa) / (len(labels) - sum(labels))
        frr = sum(fr) / sum(labels)
        delta = abs(far - frr)
        if delta < min_delta:
            min_delta = delta
            eer_accrate = accrate * 100
            eer = (far + frr) / 2
            eer_threshold = threshold
        threshold += myconfig.EVAL_THRESHOLD_STEP
    return eer, eer_threshold, eer_accrate


def save_experiment_data(file_name, far_z, frr_z):
    """保存实验数据到本地"""
    np.savez(file_name, far_z=far_z, frr_z=frr_z)


def plot_log_det_curve(labels, scores, save_to_file=True, file_name='paper/AFSC+Transformer.npz'):
    """绘制 DET 曲线并保存数据"""
    ...
    for threshold in thresholds:
        accept = [score >= threshold for score in scores]
        fa = sum(a and not l for a, l in zip(accept, labels))
        fr = sum(not a and l for a, l in zip(accept, labels))
        far = fa / (len(labels) - sum(labels))
        frr = fr / sum(labels)

        far_z.append(norm.ppf(far))  # 转换为标准正态分布的z分数
        frr_z.append(norm.ppf(frr))  # 转换为标准正态分布的z分数

    plt.figure(figsize=(8, 6))
    plt.plot(far_z, frr_z, label="DET Curve")
    plt.xlabel("False Accept Rate (log-scale)")
    plt.ylabel("False Reject Rate (log-scale)")
    plt.title("DET Curve")
    plt.legend()
    plt.grid()
    plt.show()
    ...
    if save_to_file and file_name:
        save_experiment_data(file_name, far_z, frr_z)
...
def run_eval():
    ...
    eer, eer_threshold, eer_acc = compute_eer(labels, scores)
    eval_time = time.time() - start_time
    print("Finished evaluation in", eval_time, "seconds")
    print("eer_threshold =", eer_threshold, "eer =", eer)
    print("accuracy = ", eer_acc, "%")

    plot_log_det_curve(labels, scores)
