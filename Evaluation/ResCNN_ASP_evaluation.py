...
class TripletScoreFetcher:
    ...
    def __call__(self, i):
        ...
        anchor, pos, neg = AAF_feature_extraction.get_triplet_features(self.spk_to_utts)
        ...
        if ((anchor_embedding is None) or (pos_embedding is None) or (neg_embedding is None)):
            ...
            return ([], [])
        triplet_labels = [1, 0]
        triplet_scores = [cosine_similarity(anchor_embedding, pos_embedding), cosine_similarity(anchor_embedding, neg_embedding)]
...
def run_eval():
    ...
    if myconfig.TEST_DATA_CSV:
        spk_to_utts = dataset.get_csv_spk_to_utts(myconfig.TEST_DATA_CSV)
        print("Evaluation data:", myconfig.TEST_DATA_CSV)
    else:
        spk_to_utts = dataset.get_librispeech_spk_to_utts(myconfig.TEST_DATA_DIR)
        print("Evaluation data:", myconfig.TEST_DATA_DIR)

    encoder = ResCNN_ASP_net.get_speaker_encoder(myconfig.SAVED_MODEL_PATH)
    labels, scores = compute_scores(encoder, spk_to_utts, myconfig.NUM_EVAL_TRIPLETS)
    eer, eer_threshold, eer_accrate = compute_eer(labels, scores)
    eval_time = time.time() - start_time
    print("Finished evaluation in", eval_time, "seconds")
    print("eer_threshold =", eer_threshold)
    print("eer =", eer)
    print("accuracy = ", eer_accrate, "%")
