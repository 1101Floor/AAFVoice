...
class TransformerSpeakerEncoder(BaseSpeakerEncoder):
    def __init__(self, saved_model=""):
        ...
        self.bn1 = nn.GroupNorm(num_groups=1, num_channels=myconfig.TRANSFORMER_DIM)
        self.linear1 = nn.Linear(myconfig.TRANSFORMER_DIM, myconfig.dimension)
        self.bn2 = nn.GroupNorm(num_groups=1, num_channels=myconfig.dimension)
    ...
    def forward(self, x):
        # print('x', x.shape)
        encoder_input = torch.relu(self.linear_layer(x))
        ...
        bnout1 = self.bn1(output[:, 0, :])
        out1 = self.linear1(bnout1)
        bnout2 = self.bn2(out1)
        print('正在使用Transformer')
        return bnout2
...
def get_speaker_encoder(load_from=""):
    ...
    if myconfig.USE_TRANSFORMER:
        ...
        return GRUSpeakerEncoder(load_from).to(myconfig.DEVICE)
